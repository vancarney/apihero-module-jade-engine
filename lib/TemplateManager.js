...
TemplateManager = (function(superClass) {
  ...
  TemplateManager.prototype.processConfig = function(config) {
    var getTemplate;
    this.out = 'var jade = require("../../scripts/jade-runtime.js")';
    getTemplate = (function(_this) {
      return function(template) {
        return "\n\n" + (_this[template.match(/\.jade+$/) ? 'processTemplate' : 'compileDirectory'](template));
      };
    })(this);
  ...
  TemplateManager.prototype.processTemplate = function(fileName, opts) {
    var base, e, inputString, options, refName, result;
    if (opts == null) {
      opts = {};
    }
    options = _.extend({
    ...
    if ((base = this.dependencies)[refName] == null) {
      base[refName] = [];
    }
    try {
    ...
    this.dependencies[refName] = this.dependencies[refName].concat(result.dependencies);
    result.body = result.body.replace(/^(function)\stemplate/, '$1');
    return "module.exports['" + refName + "'] = " + result.body + ";";
  };

  TemplateManager.prototype.compileDirectory = function(dir) {
    var _path, e, file, files, i, len, out, results, stat;
    ...
    for (i = 0, len = files.length; i < len; i++) {
      ...
      results.push(out += stat.isFile() ? "\n\n" + (processTemplate(_path)) : compileDirectory(path.join(dir, file)));
    }
    return results;
  };
